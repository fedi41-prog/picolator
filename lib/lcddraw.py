...
def draw_square(xx, yy, sz, cc, lcd):
    if sz > 1:
        lcd.fill_rect(xx, yy, sz, sz, cc)
    else:
        lcd.pixel(xx,yy,cc) 

def character(asc,xt,yt,sz, cc, lcd):  # Single character sz is size: 1 or 2
    code = asc * 5    # 5 bytes per character
    for ii in range(5):
        line = FONT[code + ii]
        for yy in range(8):
            if (line >> yy) & 0x1:
                draw_square(xt+ii*sz, yt+yy*sz, sz, cc, lcd)
                                    
def draw_str(asci,xx,yy,sz, cc, lcd):  # Text string
    move = sz_to_w(sz)
    
    for letter in(asci):
        asci = ord(letter)
        character(asci,xx,yy,sz, cc, lcd)
        xx = xx + move

def center_str(string, y, size, cc, lcd, offset=0): # Centres text on line y
    w = sz_to_w(size)
    gap = int((lcd.width - len(string) * w)/2)+offset
    draw_str(string, gap,y, size, cc, lcd)
    
def draw_pixels(pixels, xx, yy, cc, lcd, sz=1):
    for px in pixels:
        draw_square(px[0]*sz+xx, px[1]*sz+yy, sz, cc, lcd)
